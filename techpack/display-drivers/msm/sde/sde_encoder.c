		...
		struct msm_display_mode *msm_mode, bool pre_modeset)
{
	...
	if (pre_modeset) {
		...
		if (oplus_adfr_is_support() && (oplus_adfr_get_vsync_mode() == OPLUS_DOUBLE_TE_VSYNC)) {
			...
			if (display == NULL) {
				SDE_ERROR("invalid pointer display");
				return 0;
			}
			if (display->panel == NULL) {
				SDE_ERROR("invalid pointer panel");
				return 0;
			}
			if (sde_enc->cur_master == NULL) {
				SDE_ERROR("invalid pointer cur_master");
				switch_te = false;
				sde_enc->need_te_source_switch = true;
			}
			if (display->panel->power_mode == SDE_MODE_DPMS_LP1
					|| display->panel->power_mode == SDE_MODE_DPMS_LP2) {
				SDE_INFO("kVRR Don't switch TE in AOD mode");
				switch_te = false;
				sde_enc->need_te_source_switch = false;
			}
			if (adj_mode && switch_te) {
				vsync_source = oplus_get_vsync_source(adj_mode->hskew);
				if (vsync_source < 0) {
					SDE_ERROR("kVRR invalid vsync source");
					return 0;
				}
				...
				if (oplus_adfr_need_deferred_vsync_source_switch(drm_enc, adj_mode)) {
					...
					if (sde_enc->te_source != vsync_source) {
						SDE_INFO("kVRR vsync source switched to %d if vsync source isn't synchronized\n", vsync_source);
