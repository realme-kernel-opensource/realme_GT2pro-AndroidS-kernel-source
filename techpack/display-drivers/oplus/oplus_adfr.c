...
/* ---------- te source switch --------- */
int oplus_get_vsync_source(u32 h_skew) {
...
int oplus_vsync_source_reset(void) {
	u32 h_skew = SDC_ADFR;
	struct dsi_panel *panel;
	...
	h_skew = panel->cur_mode->timing.h_skew;
	sde_enc->te_source = oplus_get_vsync_source(h_skew);
...
bool oplus_adfr_need_deferred_vsync_source_switch(void *encoder, void *to_mode)
{
	...
	to_vsync_source = oplus_get_vsync_source(adj_mode->hskew);
	if (to_vsync_source < 0) {
