...
** Version : 1.0
** Date : 2022/01/14
** Author : zhanghua2@MM.Display.LCD Driver
**
** ------------------------------- Revision History: -----------
...
/* Add for OnScreenFingerprint dim alpha */
	...
	if (oplus_dimlayer_hbm || dimlayer_bl) {
		...
		if (aod_index >= 0) {
			...
			if (((oplus_onscreenfp_status == 1) && (panel_power_mode != SDE_MODE_DPMS_ON))
				|| (oplus_request_power_status == OFP_PMS_REQUEST_POWER_ON))
			...
			if (cstate->fingerprint_defer_sync) {
				...
				if (current_vblank == (oplus_onscreenfp_vblank_count + 1)) {
					...
					if (ktime_compare_safe(exp_ktime, vblanktime) > 0) {
						target_vblank++;
						pr_err("kOFP, hbm setting may hit into hardware irq and soft update, wait one more vblank\n");
					}
				}
				#else
				current_vblank = drm_crtc_vblank_count(crtc);
				...
				ret = wait_event_timeout(*drm_crtc_vblank_waitqueue(crtc),
						...
						msecs_to_jiffies(50));
				if (!ret)
					pr_err("kOFP, [fingerprint CRTC:%d:%s] vblank wait timed out\n",
