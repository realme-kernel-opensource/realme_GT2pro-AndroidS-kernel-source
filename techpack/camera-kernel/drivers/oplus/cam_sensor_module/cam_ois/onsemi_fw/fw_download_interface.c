...
*/
		...
		if (0 == rc)
		{
			...
			RamWrite32A(0xF01C, 0x00000011);
			IsOISReady(o_ctrl);
			CAM_INFO(CAM_OIS, "set 0xF01C 0x00000011");
			RamWrite32A(0xF016, 0x0000000A);
			IsOISReady(o_ctrl);
			CAM_INFO(CAM_OIS, "set 0xF016 0x0000000A");
...
int OIS_READ_HALL_DATA_TO_UMD_TELE124 (struct cam_ois_ctrl_t *o_ctrl,struct i2c_settings_array *i2c_settings) {
        ...
        list_for_each_entry(i2c_list,
                        ...
                        qtime_ms = ((read_buff[112]<<24)+(read_buff[113]<<16)+(read_buff[114]<<8)+read_buff[115]);
                        qtime_us = ((read_buff[116]<<24)+(read_buff[117]<<16)+(read_buff[118]<<8)+read_buff[119]);
                        if((newqtime-(fifo_count-1)*2000000)<preqtime_ms)
                                CAM_ERR(CAM_OIS,"time error");
                        preqtime_ms=newqtime;
                        CAM_DBG(CAM_OIS,"READ fifo_count=%d",fifo_count);
...
int OIS_READ_HALL_DATA_TO_UMD_129 (struct cam_ois_ctrl_t *o_ctrl,struct i2c_settings_array *i2c_settings) {
	...
	list_for_each_entry(i2c_list,
		...
		qtime_ms = ((read_buff[144]<<24)+(read_buff[145]<<16)+(read_buff[146]<<8)+read_buff[147]);
		qtime_us = ((read_buff[156]<<24)+(read_buff[157]<<16)+(read_buff[158]<<8)+read_buff[159]);
		if((newqtime-(fifo_count-1)*2000000)<preqtime_ms)
