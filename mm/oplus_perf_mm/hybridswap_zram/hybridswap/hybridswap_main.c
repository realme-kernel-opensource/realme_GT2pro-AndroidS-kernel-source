...
memcg_hybs_t *hybridswap_cache_alloc(struct mem_cgroup *memcg)
{
	memcg_hybs_t *hybs;
	u64 ret;
	...
	hybs = (memcg_hybs_t *)kmem_cache_zalloc(hybridswap_cache, GFP_KERNEL);
	if (!hybs)
		return NULL;
		...
		struct cftype *cft, s64 val)
{
	...
	if (!hybs) {
		hybs = hybridswap_cache_alloc(memcg);
		if (!hybs)
			return -EINVAL;
	}
