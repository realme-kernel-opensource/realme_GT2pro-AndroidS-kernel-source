...
#define AP_ACCESS_TO_ADSP_TIMES 0X01
...
static uint32_t fake_chg_cnt = 0;
...
bool oplus_chg_is_wls_present(void)
{
	...
	rc = oplus_chg_mod_get_property(chip->wls_ocm, OPLUS_CHG_PROP_PRESENT, &pval);
	if (rc < 0)
		return false;
...
static bool oplus_chg_is_wls_fastchg_started(void)
{
	int rc = 0;
	union oplus_chg_mod_propval pval = {0, };
	struct oplus_chg_chip *chip = g_charger_chip;
...
static bool oplus_chg_is_wls_ffc(void)
{
...
static int oplus_chg_get_wls_type(void)
{
	int rc = 0;
	union oplus_chg_mod_propval pval = {0, };
	struct oplus_chg_chip *chip = g_charger_chip;
...
static bool oplus_chg_is_wls_fw_upgrading(void)
{
	int rc = 0;
	union oplus_chg_mod_propval pval = {0, };
	struct oplus_chg_chip *chip = g_charger_chip;
...
int oplus_chg_init(struct oplus_chg_chip *chip)
{
	...
	if ((get_eng_version() == HIGH_TEMP_AGING) || (get_eng_version() == PTCRB))
		schedule_delayed_work(&chip->aging_check_work, OPLUS_CHG_UPDATE_INTERVAL);
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	if (get_eng_version() == AGING || get_boot_mode() == MSM_BOOT_MODE__AGING) {
...
static void oplus_chg_smooth_to_soc(struct oplus_chg_chip *chip)
{
	...
	if (status == DISCHARGING_STATUS) {
		if (chip->soc >= 98 && chip->soc <= 100 && chip->ui_soc == 100) {
			chip->smooth_soc = 100;
		} else {
...
static void oplus_chg_update_ui_soc(struct oplus_chg_chip *chip)
{
	...
	if ((get_eng_version() == AGING) || (get_boot_mode() == MSM_BOOT_MODE__AGING) || (get_eng_version() == HIGH_TEMP_AGING)) {
		if ((chip->ui_soc > chip->soc) && (chip->ui_soc > 10)) {
			chip->ui_soc--;
			chg_err(" HIGH_TEMP_AGING/AGING/FACTORY,ui soc decrease\n");
		}
	}
}
...
static void oplus_chg_check_status_full(struct oplus_chg_chip *chip)
{
	...
	if (chip->wireless_support
			...
			|| oplus_chg_is_wls_present())) {
		charger_xlog_printk(CHG_LOG_CRTI, "in wpc ffc or wls charging\n");
		return;
	}
	...
	charger_xlog_printk(CHG_LOG_CRTI, "fastchg_present_wait_count : %d, get_charging_enable= %d,allow_reading: %d\n",
									fastchg_present_wait_count, chip->chg_ops->get_charging_enable(), oplus_vooc_get_allow_reading());
	...
	if (oplus_vooc_get_allow_reading() == false) {
		...
		if (((oplus_vooc_get_fastchg_to_normal()== true)
			...
			charger_xlog_printk(CHG_LOG_CRTI, "fastchg_present_wait_count : %d, get_charging_enable= %d\n",
												fastchg_present_wait_count, chip->chg_ops->get_charging_enable());
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	...
	if (chip->charger_exist && (chip->charger_volt <= 2000 || chip->charger_volt >= 4000*1000)
			&& (!oplus_voocphy_get_fastchg_to_normal()) && (!oplus_voocphy_get_fastchg_to_warm())) {
			fake_chg_cnt++;
		if (fake_chg_cnt >= 2) {
			fake_chg_cnt = 0;
			charger_xlog_printk(CHG_LOG_CRTI, "A fake charging after plug-out adapter may occured!charger_exist:%d, chg_vol:%dmV , Manually call plug_irq_work",
					chip->charger_exist, chip->charger_volt);
			schedule_delayed_work(&chip->pmic_spmi.bcdev_chip->plugin_irq_work, msecs_to_jiffies(1000));
		}
	} else {
		fake_chg_cnt = 0;
	}
...
static void oplus_chg_pd_config(struct oplus_chg_chip *chip)
{
	...
	if (chip->chg_ops->oplus_chg_get_pd_type() > 0) {
		...
		chg_err("oplus_chg_pd_config:pd_type[%d] \n", pd_type);
...
static void oplus_aging_check_work(struct work_struct *work)
{
	...
	if (chip->charger_exist &&
		((get_eng_version() == HIGH_TEMP_AGING) || (get_eng_version() == PTCRB))) {
		chg_count++;
...
int oplus_chg_show_vooc_logo_ornot(void)
{
	...
	if (g_charger_chip->wireless_support
			&& (oplus_wpc_get_adapter_type() == CHARGER_SUBTYPE_FASTCHG_SVOOC
				|| oplus_chg_get_wls_type() == OPLUS_CHG_WLS_SVOOC
...
#endif
	charger_xlog_printk(CHG_LOG_CRTI, "get subtype subtype = [%d]\n", subtype);
	switch(subtype) {
	...
	case CHARGER_SUBTYPE_FASTCHG_VOOC:
		if (chip->chg_ctrl_by_vooc == true) {
			if (c_level_index <= 0) {
				...
				if (c_level_index > ARRAY_SIZE(cool_down_current_limit_normal)) {
					chip->limits.input_current_cool_down_ma =
						val <  cool_down_current_limit_normal[ARRAY_SIZE(cool_down_current_limit_normal) - 1]
						? val : cool_down_current_limit_normal[ARRAY_SIZE(cool_down_current_limit_normal) - 1];
				} else {
					chip->limits.input_current_cool_down_ma =
						val <  cool_down_current_limit_normal[c_level_index - 1]
						? val : cool_down_current_limit_normal[c_level_index - 1];
				}
				...
				if (c_level_index > ARRAY_SIZE(cool_down_current_limit_vooc)) {
					chip->limits.input_current_vooc_ma_high =
						val <  cool_down_current_limit_vooc[ARRAY_SIZE(cool_down_current_limit_vooc) - 1]
						? val : cool_down_current_limit_vooc[ARRAY_SIZE(cool_down_current_limit_vooc) - 1];
				} else {
					chip->limits.input_current_vooc_ma_high =
						val <  cool_down_current_limit_vooc[c_level_index - 1]
						? val : cool_down_current_limit_vooc[c_level_index - 1];
				}
...
void oplus_chg_get_props_from_adsp_by_buffer(void)
{
	...
	if (!chip->charger_exist && access_count < AP_ACCESS_TO_ADSP_TIMES
		&& ((chip->vbatt_num == 2 && chip->icharging < 750)
		||(chip->vbatt_num == 1 && chip->icharging < 1500))) {
		access_count++;
		return;
	}
	access_count = 0;
