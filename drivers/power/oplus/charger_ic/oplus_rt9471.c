...
static void rt9471_get_charger_type(struct rt9471_chip *chip, enum charger_type *type)
{
	enum charger_type chg_type = CHARGER_UNKNOWN;
	chg_type = hw_charging_get_charger_type();

	pr_notice("rt9471_get_charger_type:%d\n", chg_type);
	switch (chg_type) {
	case CHARGER_UNKNOWN:
...
//#ifndef CONFIG_TCPC_CLASS
	...
	if(true) {
		...
		atomic_set(&chip->vbus_gd, rt9471_is_vbusgd(chip));
		if (chip->dev_id == RT9471_DEVID) {
			rt9471_get_charger_type(chip, &chip->chg_type);
			rt9471_inform_psy_changed(chip);
		}
...
static void rt9471_init_work_handler(struct work_struct *work)
{
	...
	mutex_lock(&chip->bc12_lock);
	atomic_set(&chip->vbus_gd, rt9471_is_vbusgd(chip));
	if (chip->dev_id == RT9471_DEVID && rt9471_is_vbusgd(chip)) {
		rt9471_get_charger_type(chip, &chip->chg_type);
		rt9471_inform_psy_changed(chip);
	}
	rt9471_bc12_preprocess(chip);
