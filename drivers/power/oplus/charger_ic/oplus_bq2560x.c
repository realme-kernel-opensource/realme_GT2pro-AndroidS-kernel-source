...
#ifdef CONFIG_TCPC_CLASS
	if (bq->is_sgm41511 && en
			&& !g_oplus_chip->chg_ops->get_charging_enable()) {
		bq->oplus_chg_type = POWER_SUPPLY_TYPE_USB_DCP;
...
static void determine_initial_status(struct bq2560x *bq)
{
	...
	if ((bq->chg_type == STANDARD_HOST || bq->chg_type == CHARGING_HOST)&&(bq->chg_det_enable == true)) {
...
int oplus_bq2560x_set_aicr(int current_ma)
{
	...
	if (strcmp(g_bq->chg_dev_name, "primary_chg") == 0) {
		if((g_bq->chg_type == CHARGER_UNKNOWN) && (g_bq->chg_det_enable == true)){
			current_ma = 500;
		}
	}
