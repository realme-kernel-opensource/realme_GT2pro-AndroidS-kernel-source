...
#define pr_fmt(fmt) "[SM8350]([%s][%d]): " fmt, __func__, __LINE__
...
//#include "../gauge_ic/oplus_sm8350.h"
...
static void oppo_hvdcp_disable_work(struct work_struct *work)
{
	...
	if (chip && chip->chg_ops && chip->chg_ops->check_chrdet_status &&
	    chip->chg_ops->check_chrdet_status() == false)
...
static void oplus_otg_init_status_func(struct work_struct *work)
{
	...
	while (count--) {
		if (bcdev->is_chargepd_ready &&
			(chip->wireless_support && !oplus_wpc_check_chip_is_null()))
			break;

		if (bcdev->is_chargepd_ready && !chip->wireless_support)
			break;
...
static void oplus_otg_status_check_work(struct work_struct *work)
{
	...
	if (skin_temp < 0) {
		chg_err("Error get skin temp!, skin_temp = %d\n", skin_temp);
		return;
	}

	rc = oplus_get_bat_info_for_otg_status_check(&real_soc, &batt_current);
	if (rc < 0) {
		chg_err("Error oplus_get_bat_info_for_otg_status_check, rc = %d\n", rc);
		return;
	}

	chg_err("oplus_otg_status_check_work, batt_current = %d, skin_temp = %d, real_soc = %d\n",
...
static void oplus_vbus_enable_adc_work(struct work_struct *work)
{
	...
	if (!chip || !chip->chg_ops) {
		chg_err("chip or chip->chg_ops NULL\n");
		return;
	}
		...
		union power_supply_propval *pval)
{
	...
	if (prop_id == USB_ADAP_TYPE) {
		if (oplus_vchg_trig_is_support() == true
			...
			if (chip && chip->voocphy.fastchg_start == false
				&& chip->voocphy.fastchg_to_normal == false
				&& chip->voocphy.fastchg_to_warm == false) {
				pval->intval = 0;
			}
		...
		const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (chip && chip->smart_charging_screenoff) {
			oplus_smart_charge_by_shell_temp(chip, pval->intval);
			break;
		} else {
			return  -EINVAL;
		}
...
static int oplus_chg_8350_set_icl(struct oplus_chg_ic_dev *ic_dev, int icl_ma)
{
	struct battery_chg_dev *bcdev;
	int prop_id = 0;
	struct psy_state *pst = NULL;
...
static int oplus_chg_8350_get_charger_cycle(struct oplus_chg_ic_dev *ic_dev, int *cycle)
{
	...
	if (ic_dev == NULL || chip == NULL) {
		chg_err("oplus_chg_ic_dev or chip is NULL");
		return -ENODEV;
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static void *oplus_chg_8350_buck_get_func(struct oplus_chg_ic_dev *ic_dev,
					  enum oplus_chg_ic_func func_id)
{
	...
	case OPLUS_IC_FUNC_VOOCPHY_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_VOOCPHY_ENABLE,
					       oplus_chg_8350_voocphy_enable);
		break;
	case OPLUS_IC_FUNC_VOOCPHY_RESET_AGAIN:
		func = OPLUS_CHG_IC_FUNC_CHECK(
			OPLUS_IC_FUNC_VOOCPHY_RESET_AGAIN,
			oplus_chg_8350_voocphy_reset_again);
		break;
	case OPLUS_IC_FUNC_GET_CHARGER_CYCLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(
			OPLUS_IC_FUNC_GET_CHARGER_CYCLE,
			oplus_chg_8350_get_charger_cycle);
		break;
	case OPLUS_IC_FUNC_WLS_BOOST_ENABLE:
		func = OPLUS_CHG_IC_FUNC_CHECK(OPLUS_IC_FUNC_WLS_BOOST_ENABLE,
					       oplus_chg_8350_wls_boost_enable);
		break;
	case OPLUS_IC_FUNC_SET_WLS_BOOST_VOL:
		func = OPLUS_CHG_IC_FUNC_CHECK(
			OPLUS_IC_FUNC_SET_WLS_BOOST_VOL,
			oplus_chg_8350_set_wls_boost_vol);
		break;
...
static int oplus_sm8350_ic_register(struct battery_chg_dev *bcdev)
{
	...
	for_each_child_of_node(bcdev->dev->of_node, child) {
		...
		ic_dev = devm_oplus_chg_ic_register(bcdev->dev, child->name, ic_index);
		if (!ic_dev) {
			rc = -ENODEV;
		...
		case OPLUS_CHG_IC_BUCK:
			...
			sprintf(ic_dev->manu_name, "buck-sm8350\0");
			sprintf(ic_dev->fw_id, "0x00\0");
			ic_dev->get_func = oplus_chg_8350_buck_get_func;
			ic_dev->virq_data = oplus_chg_8350_buck_virq_table;
			ic_dev->virq_num = ARRAY_SIZE(oplus_chg_8350_buck_virq_table);
			schedule_delayed_work(&bcdev->child_register_work, 0);
			break;
		case OPLUS_CHG_IC_GAUGE:
			bcdev->gauge_ic = ic_dev;
			sprintf(ic_dev->manu_name, "adsp-gauge\0");
			sprintf(ic_dev->fw_id, "0x00\0");
			ic_dev->get_func = oplus_chg_8350_gauge_get_func;
			ic_dev->virq_data = oplus_chg_8350_gauge_virq_table;
			ic_dev->virq_num = ARRAY_SIZE(oplus_chg_8350_gauge_virq_table);
...
static void oplus_child_register_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct battery_chg_dev *bcdev = container_of(dwork,
				struct battery_chg_dev, child_register_work);
	...
	if (gauge_topic == NULL) {
		if (retry > 0) {
			retry--;
			schedule_delayed_work(&bcdev->child_register_work,
...
static int battery_chg_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&bcdev->child_register_work, oplus_child_register_work);
#endif
