			   ...
			   u8 fw_size)
{
	...
	msleep(2);
	oplus_i2c_dma_write(chip->client, REG_RESET, 4, (u8 *)(&sw_reset_flag));
	sprintf(chip->ic_dev->fw_id, "0x%x\0", fw_buf[fw_size - 4]);
	chg_debug("success\n");
	return 0;
...
static int rk826_get_fw_verion_from_ic(struct rk826_chip *chip)
{
	...
	if (oplus_is_power_off_charging(NULL) || oplus_is_charger_reboot(NULL)) {
		chip->upgrading = true;
...
static int __rk826_fw_check_then_recover(struct rk826_chip *chip)
{
	...
	if (oplus_is_power_off_charging(NULL) == true ||
	    oplus_is_charger_reboot(NULL) == true) {
		chip->upgrading = true;
...
static int rk826_parse_fw_data(struct rk826_chip *chip)
{
			      ...
			      const struct i2c_device_id *id)
{
	...
	chg_info("register %s\n", node->name);
	sprintf(chip->ic_dev->manu_name, "rk826\0");
	sprintf(chip->ic_dev->fw_id, "0x00\0");
	chip->ic_dev->type = ic_type;
	chip->ic_dev->get_func = rk826_get_func;
	chip->ic_dev->virq_data = rk826_virq_table;
	chip->ic_dev->virq_num = ARRAY_SIZE(rk826_virq_table);
