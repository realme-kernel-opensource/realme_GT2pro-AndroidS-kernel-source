...
__maybe_unused static int bq27541_get_gauge_i2c_err(void)
{
	if (!gauge_ic) {
		return 0;
	}
...
__maybe_unused static void bq27541_clear_gauge_i2c_err(void)
{
	if (!gauge_ic) {
...
static void bq27541_reset(struct i2c_client *client)
{
	...
	if (bq27541_get_battery_mvolts() <= 3300
			&& bq27541_get_battery_mvolts() > 2500
			&& ui_soc == 0
...
*/
	...
	fg_ic->ic_dev =
		devm_oplus_chg_ic_register(fg_ic->dev, fg_ic->dev->of_node->name, ic_index);
	if (!fg_ic->ic_dev) {
		rc = -ENODEV;
	...
	case DEVICE_BQ27541:
		sprintf(fg_ic->ic_dev->manu_name, "bq27541\0");
		sprintf(fg_ic->ic_dev->fw_id, "0x00\0");
		break;
	case DEVICE_BQ27411:
		sprintf(fg_ic->ic_dev->manu_name, "bq27411\0");
		sprintf(fg_ic->ic_dev->fw_id, "0x00\0");
		break;
	case DEVICE_ZY0602:
		sprintf(fg_ic->ic_dev->manu_name, "zy0602\0");
		sprintf(fg_ic->ic_dev->fw_id, "0x00\0");
		break;
	default:
		sprintf(fg_ic->ic_dev->manu_name, "UNKNOWN\0");
		sprintf(fg_ic->ic_dev->fw_id, "0x00\0");
		break;
	}
	fg_ic->ic_dev->type = ic_type;
	fg_ic->ic_dev->get_func = oplus_chg_get_func;
	fg_ic->ic_dev->virq_data = bq27541_virq_table;
	fg_ic->ic_dev->virq_num = ARRAY_SIZE(bq27541_virq_table);
