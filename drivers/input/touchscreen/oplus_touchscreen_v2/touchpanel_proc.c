...
/*touch_v2_switch_enable
 * Input:
 * touch_v2:0, disable the touch_v2_switch;
 * touch_v2:a, enable the touch_v2_switch;
 */
static ssize_t proc_touch_v2_switch_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
	int value = 0;
	char buf[5] = {0};
	struct touchpanel_data *ts = PDE_DATA(file_inode(file));
	...
	if (ts->smooth_level_array_support && ts->ts_ops->smooth_lv_set) {
		if (value != 0) {
			ts->smooth_level_used_array = (u32 *)&(ts->smooth_v2_level_array);
		} else {
	...
	if (ts->sensitive_level_array_support && ts->ts_ops->sensitive_lv_set) {
		if (value != 0) {
			ts->sensitive_level_used_array = (u32 *)&(ts->sensitive_v2_level_array);
		} else {
...
static ssize_t proc_touch_v2_switch_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	int ret = 0;
	char page[PAGESIZE] = {0};
	struct touchpanel_data *ts = PDE_DATA(file_inode(file));

	if (!ts || !ts->smooth_level_array_support || !ts->sensitive_level_array_support) {
		snprintf(page, PAGESIZE - 1, "%d\n", -1);
	} else {
		snprintf(page, PAGESIZE - 1, "%d\n", ts->touch_v2);
	}
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0)
	static const struct proc_ops proc_touch_v2_switch_fops = {
		.proc_write = proc_touch_v2_switch_write,
		.proc_read	= proc_touch_v2_switch_read,
		.proc_open	= simple_open,
	};
#else
	static const struct file_operations proc_touch_v2_switch_fops = {
		.write = proc_touch_v2_switch_write,
		.read  = proc_touch_v2_switch_read,
		.open  = simple_open,
		.owner = THIS_MODULE,
	};
#endif
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,10,0)
static const struct proc_ops tp_health_monitor_proc_fops = {
...
int init_touchpanel_proc(struct touchpanel_data *ts)
{
	...
	tp_proc_node tp_proc_node[] = {
		...
		{
			"adaptive_switch_enable", 0666, NULL, &proc_touch_v2_switch_fops, ts, false,
			ts->touch_v2_switch_support
		},
