...
static int init_parse_dts(struct device *dev, struct touchpanel_data *ts)
{
	...
	ts->touch_v2_switch_support 	= of_property_read_bool(np, "touch_v2_switch_support");
	...
	if (rc) {
		...
		rc = of_property_read_u32_array(np,
						"touchpanel,smooth-v2-level",
						temp_array,
						SMOOTH_LEVEL_NUM);
		if (rc) {
			TP_INFO(ts->tp_index, "smooth_v2_level_array not specified %d\n", rc);
			for (i=0; i < SMOOTH_LEVEL_NUM; i++) {
				ts->smooth_v2_level_array[i] = ts->smooth_level_array[i];
			}
		} else {
			for (i=0; i < SMOOTH_LEVEL_NUM; i++) {
				ts->smooth_v2_level_array[i] = temp_array[i];
			}
		}
		ts->smooth_level_used_array = (u32 *)&(ts->smooth_level_array);
	}
	...
	if (rc) {
		...
		rc = of_property_read_u32_array(np,
						"touchpanel,sensitive-v2-level-charging",
						temp_array,
						SENSITIVE_LEVEL_NUM);
		if (rc) {
			TP_INFO(ts->tp_index, "sensitive_v2_charging_array not specified %d\n", rc);
			for (i=0; i < SENSITIVE_LEVEL_NUM; i++) {
				ts->sensitive_v2_level_array[i] = ts->sensitive_level_array[i];
			}
		} else {
			for (i=0; i < SENSITIVE_LEVEL_NUM; i++) {
				ts->sensitive_v2_level_array[i] = temp_array[i];
			}
		}
...
#ifdef CONFIG_OPLUS_MTK_DRM_GKI_NOTIFY
MODULE_SOFTDEP("pre:oplus_bsp_tp_comon");
