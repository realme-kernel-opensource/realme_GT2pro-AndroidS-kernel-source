...
#define BAD_GAME_RTT_THREAD 180
...
void oplus_score_calc_and_report(void)
{
	...
	for (i = 0; i < MAX_LINK_NUM; i++) {
		...
		if (uplink_total_packets == 0) {
			...
			} else if (uplink_srtt > BAD_GAME_RTT_THREAD && (uplink_total_packets || downlink_total_packets)
						&& (uplink_total_packets < 10) && (downlink_total_packets < 10)) {
